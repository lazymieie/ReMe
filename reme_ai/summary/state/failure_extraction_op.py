"""Failure extraction operation for state memory generation."""
...
@C.register_op()
class StateFailureExtractionOp(BaseAsyncOp):
    """Extract failure-prevention state-level experiences from trajectories."""
    ...
    async def async_execute(self):
        ...
        if not failure_trajectories:
            logger.info("No failure trajectories found for state extraction")
            return

        failure_state_memories = []
        for trajectory in failure_trajectories:
            segments = trajectory.metadata.get("segments", [])
            if segments:
                for segment in segments:
                    failure_state_memories.extend(await self._extract_from_steps(segment, trajectory))
            else:
                failure_state_memories.extend(await self._extract_from_steps(trajectory.messages, trajectory))

        self.context.failure_state_memories = failure_state_memories
        logger.info(f"Extracted {len(failure_state_memories)} failure state memories")
    ...
    async def _extract_from_steps(self, steps: List[Message], trajectory: Trajectory) -> List[BaseMemory]:
        ...
        prompt = self.prompt_format(
            prompt_name="failure_step_state_memory_prompt",
